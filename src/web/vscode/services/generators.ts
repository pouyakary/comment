...
export function executeGenerator(
  ...
  const userSettings = parameters.getUserSettings(editorComputationOptions);
  const editorParameters = parameters.computeEnvironmentalParameters(
    editorComputationOptions,
  );

  const languageConfig = languages.loadLanguageSettings(
    editorParameters.languageId,
  );
  if (languageConfig === null) {
    throw Error(
      `Language '${editorParameters.languageId} is not supported. Please make an issue in GitHub to add the support for it.`,
    );
  }
